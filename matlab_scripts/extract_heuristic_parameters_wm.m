...
d_heu_min = find(dx_vec>=1.70, 1 );
d_heu_max = find(dx_vec<=2.20, 1, 'last' );
...
%% WM HEURISTIC PARAMETER ESTIMATION AND DMRI METRIC SMOOTH DISTRIBUTION VISUALIZATION 
% Initialize figure 1.
h(1).fig = figure(1);
set(h(1).fig, 'Position', [50, 50, 1200, 950]);

% Deviation of FA WM heuristic parameters and their group-based
% distribution plots, i.e. Fig. 3 in Labounek et al. (2020) Scientific Reports
heu_sbj = [];
heu_prot = []; 
heu_FA = [];
heu_FA2 = [];
for prot = 1:size(FA_wm_kernel,3)
    pdfs = FA_wm_kernel(:,:,prot);
    pdfs_sum = sum(pdfs);
    pdfs_sbj = sbj;
    pdfs_sbj(pdfs_sum == 0) = [];
    pdfs(:,pdfs_sum == 0) = [];
    ...
    table_vec(indxs,1) = heu_FA_prot;
    
    if prot == 1
        table_ZOOMit_Int(:,70) = table_vec;
    elseif prot == 2
        table_ZOOMit_NotInt(:,70) = table_vec;
    elseif prot == 3
        table_RESOLVE(:,70) = table_vec;
    end
...
% Deviation of f1 WM heuristic parameters and their group-based
% distribution plots, i.e. Fig. 3 in Labounek et al. (2020) Scientific Reports
heu_f1 = [];
heu_f12 = [];
for prot = 1:size(f1_wm_kernel,3)
    pdfs = f1_wm_kernel(:,:,prot);
    pdfs_sum = sum(pdfs);
    pdfs_sbj = sbj;
    pdfs_sbj(pdfs_sum == 0) = [];
    pdfs(:,pdfs_sum == 0) = [];
    ...
    table_vec = zeros(size(FA_wm_mean,1),1);
    table_vec(indxs,1) = heu_f1_prot;
    
    if prot == 1
        table_ZOOMit_Int(:,71) = table_vec;
    elseif prot == 2
        table_ZOOMit_NotInt(:,71) = table_vec;
    elseif prot == 3
        table_RESOLVE(:,71) = table_vec;
    end
...
% Deviation of d WM heuristic parameters and their group-based
% distribution plots, i.e. Fig. 3 in Labounek et al. (2020) Scientific Reports
heu_d = [];
for prot = 1:size(d_wm_kernel,3)
    pdfs = d_wm_kernel(:,:,prot);
    pdfs_sum = sum(pdfs);
    pdfs_sbj = sbj;
    pdfs_sbj(pdfs_sum == 0) = [];
    pdfs(:,pdfs_sum == 0) = [];
    ...
    if prot == 1
        table_ZOOMit_Int(:,73) = table_vec;
    elseif prot == 2
        table_ZOOMit_NotInt(:,73) = table_vec;
    elseif prot == 3
        table_RESOLVE(:,73) = table_vec;
    end
...
% Deviation of MD WM heuristic parameters and their group-based
% distribution plots, i.e. Fig. 3 in Labounek et al. (2020) Scientific Reports
heu_MD = [];
for prot = 1:size(MD_wm_kernel,3)
    pdfs = MD_wm_kernel(:,:,prot);
    pdfs_sum = sum(pdfs);
    pdfs_sbj = sbj;
    pdfs_sbj(pdfs_sum == 0) = [];
    pdfs(:,pdfs_sum == 0) = [];
    ...
    if prot == 1
        table_ZOOMit_Int(:,72) = table_vec;
    elseif prot == 2
        table_ZOOMit_NotInt(:,72) = table_vec;
    elseif prot == 3
        table_RESOLVE(:,72) = table_vec;
    end
...
pause(3)
print(fullfile(save_path,'dmri_distributions_wm'), '-dpng', '-r300')
close(h(1).fig)
pause(1)

%% VISUALIZATION OF FA HEURISTIC PARAMETER DISTRIBUTIONS FROM WM
...
table_pvals(75,:) = [pCR_ZOOMint pCP_ZOOMint pRP_ZOOMint pSC_ZOOMint pSR_ZOOMint pSP_ZOOMint ...
...
hold off
title({'FA heuristic parameter probability distribution from WM';'C3-C6 segments over subjects'})
...
grid on
pause(1)
print(fullfile(save_path,[num2str(size(subject,1)) '_subjects_FA_heu_WM_over_subjects']), '-dpng', '-r300')

%% VISUALIZATION OF f1 HEURISTIC PARAMETER DISTRIBUTIONS FROM WM
...
table_pvals(76,:) = [pCR_ZOOMint pCP_ZOOMint pRP_ZOOMint pSC_ZOOMint pSR_ZOOMint pSP_ZOOMint ...
        pCR_ZOOMnotint pCP_ZOOMnotint pRP_ZOOMnotint pSC_ZOOMnotint pSR_ZOOMnotint pSP_ZOOMnotint ...
        pCR_RESOLVE pCP_RESOLVE pRP_RESOLVE pSC_RESOLVE pSR_RESOLVE pSP_RESOLVE];
h(3).fig = figure(3);
...
hold off
title({'f_1 heuristic parameter probability distribution from WM';'C3-C6 segments over subjects'})
set(gca,'XTick',(1:1:3)/2,...
     'XTickLabel',{'ZOOMit interp'
                   'ZOOMit non-interp'
                   'RESOLVE'
...
grid on
pause(1)
print(fullfile(save_path,[num2str(size(subject,1)) '_subjects_f1_heu_WM_over_subjects']), '-dpng', '-r300')

%% VISUALIZATION OF MD HEURISTIC PARAMETER DISTRIBUTIONS FROM WM
...
table_pvals(77,:) = [pCR_ZOOMint pCP_ZOOMint pRP_ZOOMint pSC_ZOOMint pSR_ZOOMint pSP_ZOOMint ...
        pCR_ZOOMnotint pCP_ZOOMnotint pRP_ZOOMnotint pSC_ZOOMnotint pSR_ZOOMnotint pSP_ZOOMnotint ...
        pCR_RESOLVE pCP_RESOLVE pRP_RESOLVE pSC_RESOLVE pSR_RESOLVE pSP_RESOLVE];
h(4).fig = figure(4);
...
if pCR_ZOOMint < WillThr
    plot((1-0.31)/2,0.985,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15);
    text((1-0.25)/2,0.985,['CR p = ' num2str(round(pCR_ZOOMint*100000)/100000,4)],'FontSize',14);
end
if pCP_ZOOMint < WillThr
    plot((1-0.31)/2,0.960,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((1-0.25)/2,0.960,['CM p = ' num2str(round(pCP_ZOOMint*100000)/100000,4)],'FontSize',14);
end
if pRP_ZOOMint < WillThr
    plot((1-0.31)/2,0.935,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((1-0.25)/2,0.935,['RM p = ' num2str(round(pRP_ZOOMint*100000)/100000,4)],'FontSize',14);
end
if pSC_ZOOMint < WillThr
...
if pSR_ZOOMint < WillThr
    plot((1-0.31)/2,0.885,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((1-0.25)/2,0.885,['RS p = ' num2str(round(pSR_ZOOMint*100000)/100000,4)],'FontSize',14);
end
if pSP_ZOOMint < WillThr
    plot((1-0.31)/2,0.860,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
...
if pCR_ZOOMnotint < WillThr
    plot((2-0.31)/2,0.985,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15);
    text((2-0.25)/2,0.985,['CR p = ' num2str(round(pCR_ZOOMnotint*100000)/100000,4)],'FontSize',14);
end
if pCP_ZOOMnotint < WillThr
    plot((2-0.31)/2,0.960,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((2-0.25)/2,0.960,['CM p = ' num2str(round(pCP_ZOOMnotint*100000)/100000,4)],'FontSize',14);
end
if pRP_ZOOMnotint < WillThr
    plot((2-0.31)/2,0.935,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((2-0.25)/2,0.935,['RM p = ' num2str(round(pRP_ZOOMnotint*100000)/100000,4)],'FontSize',14);
end
if pSC_ZOOMnotint < WillThr
...
if pSR_ZOOMnotint < WillThr
    plot((2-0.31)/2,0.885,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((2-0.25)/2,0.885,['RS p = ' num2str(round(pSR_ZOOMnotint*100000)/100000,4)],'FontSize',14);
end
if pSP_ZOOMnotint < WillThr
    plot((2-0.31)/2,0.860,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
...
if pCR_RESOLVE < WillThr
    plot((3-0.37)/2,0.985,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15);
    text((3-0.31)/2,0.985,['CR p = ' num2str(round(pCR_RESOLVE*100000)/100000,4)],'FontSize',14);
end
if pCP_RESOLVE < WillThr
    plot((3-0.37)/2,0.960,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((3-0.31)/2,0.960,['CM p = ' num2str(round(pCP_RESOLVE*100000)/100000,4)],'FontSize',14);
end
if pRP_RESOLVE < WillThr
    plot((3-0.37)/2,0.935,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((3-0.31)/2,0.935,['RM p = ' num2str(round(pRP_RESOLVE*100000)/100000,4)],'FontSize',14);
end
if pSC_RESOLVE < WillThr
    plot((3-0.37)/2,0.910,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((3-0.31)/2,0.910,['CS p = ' num2str(round(pSC_RESOLVE*100000)/100000,4)],'FontSize',14);
end
if pSR_RESOLVE < WillThr
    plot((3-0.37)/2,0.885,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
    text((3-0.31)/2,0.885,['RS p = ' num2str(round(pSR_RESOLVE*100000)/100000,4)],'FontSize',14);
end
if pSP_RESOLVE < WillThr
    plot((3-0.37)/2,0.860,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15)
...
hold off
title({'MD heuristic parameter probability distribution from WM';'C3-C6 segments over subjects'})
set(gca,'XTick',(1:1:3)/2,...
     'XTickLabel',{'ZOOMit interp'
                   'ZOOMit non-interp'
                   'RESOLVE'
...
axis([0.5/2 3.5/2 0.2 1.00])
% axis([0.25 1.75 0.2 0.9])
grid on
pause(1)
print(fullfile(save_path,[num2str(size(subject,1)) '_subjects_MD_heu_WM_over_subjects']), '-dpng', '-r300')

%% VISUALIZATION OF d HEURISTIC PARAMETER DISTRIBUTIONS FROM WM
...
table_pvals(78,:) = [pCR_ZOOMint pCP_ZOOMint pRP_ZOOMint pSC_ZOOMint pSR_ZOOMint pSP_ZOOMint ...
        pCR_ZOOMnotint pCP_ZOOMnotint pRP_ZOOMnotint pSC_ZOOMnotint pSR_ZOOMnotint pSP_ZOOMnotint ...
        pCR_RESOLVE pCP_RESOLVE pRP_RESOLVE pSC_RESOLVE pSR_RESOLVE pSP_RESOLVE];
h(5).fig = figure(5);
...
%     'Controls','Mild comp. asym. patients','Severe comp. asym. patients'})
if pCR_ZOOMint < WillThr
    plot((1-0.31)/2,0.985,'k*','LineStyle','none','LineWidth',3,'MarkerSize',15);
    text((1-0.25)/2,0.985,['CR p = ' num2str(round(pCR_ZOOMint*100000)/100000,4)],'FontSize',14);
end
...
hold off
title({'d heuristic parameter probability distribution from WM';'C3-C6 segments over subjects'})
set(gca,'XTick',(1:1:3)/2,...
     'XTickLabel',{'ZOOMit interp'
                   'ZOOMit non-interp'
                   'RESOLVE'
...
axis([0.5/2 3.5/2 0.1 1.00])
% axis([0.25 1.75 0.2 0.9])
grid on
pause(1)
print(fullfile(save_path,[num2str(size(subject,1)) '_subjects_d_heu_WM_over_subjects']), '-dpng', '-r300')

%% STORING WM HEURISTIC RESULTS INTO WM SPECIFIC HEU VARIABLES
heu_f1WM=heu_f1;
heu_FAWM=heu_FA;
heu_MDWM=heu_MD;
heu_dWM=heu_d;
%% SAVE NEW WM HEURISTIC RESULTS INTO THE ORIGINAL workspace_file
% Now the file is ready for calling a sript adding GM herustic parameters
% and then all derived dmri-metrics will be ready and stored for further analyses.
close all
pause(1);
save(workspace_file)